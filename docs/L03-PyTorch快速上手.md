...
class TinyLM(nn.Module):
    ...
    def forward(self, x):
        ...
        logits = self.linear2(h)         # 向量 → 词表大小的分数     self.linear2: 输出层。将向量维度映射回 vocab_size，输出对每个字符的“预测分数”（Logits）。
        return logits
...
目的：这是当前的输入。在后续的循环中，模型会接收这个 current，预测下一个字符；然后把预测到的新字符再变成新的 current 喂给模型，循环往复。
    ...
    for _ in range(20):含义：让模型连续预测 20 个字符。_ 是一个占位符，表示我们只关心循环次数，不关心当前的索引值。
        ...
        next_id = torch.multinomial(probs, 1).item()      torch.multinomial(probs, 1)：按概率随机抽1个并返回索引，.item() 是将只有一个元素的张量转换成标准的 Python 数字。
        result.append(id_to_char[next_id])通过 id_to_char 映射表，把选中的数字（ID）变回人类看得懂的字符（如 'e'），然后装进 result 列表里。
        current = torch.tensor([next_id]) 开始套娃
